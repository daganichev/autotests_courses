...
class PersonInfo:
    ...
    # Следующий метод высчитывает и возвращает фамилию и имя в формате Фамилия И.
    def short_name(self):
        name = self.name
        name_list = name.split()
        return name_list[-1] + ' ' + name_list[0][0] + '.'

    # Следующий метод возвращает подразделение в формате "Головное подразделение --> ... --> Конечное подразделение"
    def path_deps(self):
        subdivision = ' --> '.join(self.subdivision)
        return subdivision

    """ Следующий метод записывает все подразделения в строку
        проходясь по строке записывает все символы в словарь
        в цикле высчитывает количество каждого  символа в строке
        записывает в список числовые значения всех символов и сортирует список
        после чего возвращает значение новой зарплаты высчитанной по формуле
    """
    def new_salary(self):
        subdivision_string = ''.join(self.subdivision)
        quantity_symbols = []
        letters_dict = dict.fromkeys([i for i in subdivision_string], 0)
        for i in subdivision_string:
            letters_dict[i] = letters_dict.get(i) + 1
        quantity_symbols += letters_dict.values()
        quantity_symbols.sort()
        return 1337 * self.age * sum(quantity_symbols[-3:])
